app.controller('PreVerificationPendingCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.PreVerificationPending = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.PreVerificationPending.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getPreVerificationPending();
        }
        else {
            $localStorage.Stats = null;
            $scope.PreVerificationPending = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/PreVerificationPending/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/PreVerificationPending/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getPreVerificationPending = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.PreVerificationPending.ProgrammeInstancePartTermId == "" ||
            $scope.PreVerificationPending.ProgrammeInstancePartTermId == null ||
            $scope.PreVerificationPending.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/PreVerificationPending/PendingforVerificationByFacultyGet',
            data: { ProgrammeInstancePartTermId: $scope.PreVerificationPending.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "PreVerificationPendingByFacultyReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'PreVerification Pending By Faculty Report | Date and Time: ' + DateAndTime,
            },
            columns: [
