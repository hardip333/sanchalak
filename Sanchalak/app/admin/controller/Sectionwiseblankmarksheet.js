app.controller('SectionwiseblankmarksheetCtrl', function ($scope, $http, $rootScope, $filter, $localStorage, $state, $cookies, $mdDialog, NgTableParams, $window) {
    ...
    $scope.getPaperListForBlankMarksheet = function () {
        ...
        $http({
            method: 'POST',
            url: 'api/Sectionwiseblankmarksheet/getPaperListForBlankMarksheet',
            data: $scope.filter,
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    $scope.SN = $scope.details.SectionName;
                    alert($scope.SN);
    ...
    $scope.downloadBlankMarksheetForUA = function (data) {
        debugger;
            ...
            $scope.ItemNew = {};
            $scope.ItemNew.PaperId = data.Id;
            $scope.ItemNew.ExamMasterId = $scope.filter.ExamMasterId;
            $scope.ItemNew.ProgrammePartTermId = $scope.filter.ProgrammePartTermId;
            $scope.ItemNew.BranchId = $scope.filter.BranchId;
            $scope.ItemNew.TeachingLearningMethodId = $scope.filter.TeachingLearningMethodId;
            $scope.ItemNew.AssessmentMethodId = $scope.filter.AssessmentMethodId;
            $scope.ItemNew.SectionName = data.SectionName;
            $scope.ItemNew.SectionMarks = data.SectionMarks;
        
        debugger
        $scope.onSpinner();
        $http({
            method: 'POST',
            url: 'api/Sectionwiseblankmarksheet/generateBlankMarkSheetSectionwise',
            data: $scope.ItemNew,
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
