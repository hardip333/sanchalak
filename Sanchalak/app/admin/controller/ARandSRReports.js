app.controller('SRARCtrl', function ($scope, $http, $rootScope, $state, $cookies, $mdDialog, NgTableParams, $localStorage) {
    ...
    $scope.GetProgrammeAssessmentComplete = function () {
        $scope.PartTermData = $localStorage.ARData;
        for (var i in $scope.PartTermData) {
            $scope.PartTermData[i].items = {};
        }
        $scope.PTTableParams = new NgTableParams({}, { dataset: $scope.PartTermData });
    ...
    $scope.CompletePT = function (PT) {
        ...
        //    }
        //}
        if (PT.GroupCompleteStatus == true && PT.IsCompleted == false) {
            $http({
                method: 'POST',
    ...
    $scope.CompleteAssessmentPT = function (PT) {
        ...
        //    }
        //}
        if (PT.AssessmentCompleteStatus == true && PT.TLMcheck == true && PT.IsCompletedAssessment == false) {
            $http({
                method: 'POST',
                url: 'api/GroupAndPaperReport/PTAssessmentCompleteStatusTrue',
                data: PT,
                headers: { "Content-Type": 'application/json' }
            })
                .success(function (response) {
                    ...
                    if (response.response_code == "0") {
                        ...
                        for (var i in $scope.PTTableParams.data) {
                            ...
                            if ($scope.PTTableParams.data[i].Id == PT.Id) {
                                $scope.PTTableParams.data[i].IsCompletedAssessment = true;
                            }
                        }
                    }
                })
