app.controller('CancelledByAcademicsCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.CancelledByAcademics = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.CancelledByAcademics.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getCancelledByAcademics();
        }
        else {
            $localStorage.Stats = null;
            $scope.CancelledByAcademics = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/CancelledByAcademics/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/CancelledByAcademics/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getCancelledByAcademics = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.CancelledByAcademics.ProgrammeInstancePartTermId == "" ||
            $scope.CancelledByAcademics.ProgrammeInstancePartTermId == null ||
            $scope.CancelledByAcademics.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/CancelledByAcademics/CancelledByAcademicsGet',
            data: { ProgrammeInstancePartTermId: $scope.CancelledByAcademics.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.CancelledByAcademicsTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
