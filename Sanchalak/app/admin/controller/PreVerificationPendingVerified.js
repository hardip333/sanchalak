app.controller('PreVerificationPendingVerifiedCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.PreVerificationPendingVerified = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.PreVerificationPendingVerified.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getPreVerificationPendingVerified();
        }
        else {
            $localStorage.Stats = null;
            $scope.PreVerificationPendingVerified = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/PreVerificationPendingVerified/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/PreVerificationPendingVerified/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getPreVerificationPendingVerified = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.PreVerificationPendingVerified.ProgrammeInstancePartTermId == "" ||
            $scope.PreVerificationPendingVerified.ProgrammeInstancePartTermId == null ||
            $scope.PreVerificationPendingVerified.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/PreVerificationPendingVerified/PreVerificationPendingVerifiedGet',
            data: { ProgrammeInstancePartTermId: $scope.PreVerificationPendingVerified.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.PreVerificationPendingVerifiedTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "PreVerificationPendingVerifiedFacultyReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'PreVerification Pending Verfication By Faculty Report | Date and Time: ' + DateAndTime,
            },
            columns: [
