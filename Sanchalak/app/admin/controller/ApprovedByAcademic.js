app.controller('ApprovedByAcademicCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.ApprovedByAcademic = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.ApprovedByAcademic.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getApprovedByAcademic();
        }
        else {
            $localStorage.Stats = null;
            $scope.ApprovedByAcademic = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/ApprovedByAcademic/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/ApprovedByAcademic/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getApprovedByAcademic = function () {

        //alert($scope.ApprovedByAcademic.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.ApprovedByAcademic.ProgrammeInstancePartTermId == "" ||
            $scope.ApprovedByAcademic.ProgrammeInstancePartTermId == null ||
            $scope.ApprovedByAcademic.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/ApprovedByAcademic/ApprovedByAcademicGet',
            data: { ProgrammeInstancePartTermId: $scope.ApprovedByAcademic.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.ApprovedByAcademicTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "ApprovedByAcademicsReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'Approved By Academics Report | Date and Time: ' + DateAndTime,
            },
            columns: [
