app.controller('SRARFacCtrl', function ($scope, $http, $rootScope, $state, $cookies, $mdDialog, NgTableParams, $localStorage) {
    ...
    $scope.ProgrammeGet = function () {
        ...
        //alert($scope.Institute.InstituteId);
        //$scope.ProgrammeList = {};
        $http({
            method: 'POST',
            url: 'api/GroupAndPaperReport/MstProgrammeGetByInstId',
            data: { InstituteId: $scope.Institute.InstituteId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                $rootScope.showLoading = false;
    ...
    $scope.getSR = function () {
        ...
        $state.go('StructureReportFaculty');
    };
    $scope.getAR = function () {
        ...
        $state.go('AssessmentReportFaculty');
    };
    $scope.backToList = function () {

        $state.go('GetGAPReportFaculty');
    };
});