app.controller('CancelledByStudentCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.CancelledByStudent = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.CancelledByStudent.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getCancelledByStudent();
        }
        else {
            $localStorage.Stats = null;
            $scope.CancelledByStudent = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/CancelledByStudent/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/CancelledByStudent/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getCancelledByStudent = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.CancelledByStudent.ProgrammeInstancePartTermId == "" ||
            $scope.CancelledByStudent.ProgrammeInstancePartTermId == null ||
            $scope.CancelledByStudent.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/CancelledByStudent/CancelledByStudentGet',
            data: { ProgrammeInstancePartTermId: $scope.CancelledByStudent.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.CancelledByStudentTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var mystyleSecond = {
            ...
            columns: [
                ...
                { columnid: 'InstancePartTermName', title: 'Part Term Name' },
                ...
                { columnid: 'CancelledByStudentOns', title: 'Cancelled By Student On' },
                { columnid: 'StudentCancelledRemark', title: 'Student Cancelled Remark' },
                { columnid: 'CanclledByStudent', title: 'Is Cancelled By Student?' },
    ...
    $scope.backToList = function (InstituteId) {

        debugger;
        if (InstituteId = $scope.CancelledByStudent.InstituteId) {
