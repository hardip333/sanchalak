app.controller('NotApprovedByAcademicCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.NotApprovedByAcademic = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.NotApprovedByAcademic.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getNotApprovedByAcademic();
        }
        else {
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/NotApprovedByAcademic/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/NotApprovedByAcademic/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getNotApprovedByAcademic = function () {

        //alert($scope.ApprovedByAcademic.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.NotApprovedByAcademic.ProgrammeInstancePartTermId == "" ||
            $scope.NotApprovedByAcademic.ProgrammeInstancePartTermId == null ||
            $scope.NotApprovedByAcademic.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/NotApprovedByAcademic/NotApprovedByAcademicGet',
            data: { ProgrammeInstancePartTermId: $scope.NotApprovedByAcademic.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.NotApprovedByAcademicTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "NotApprovedByAcademicsReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'Not Approved By Academics Report | Date and Time: ' + DateAndTime,
            },
            columns: [
