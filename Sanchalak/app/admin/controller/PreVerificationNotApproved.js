app.controller('PreVerificationNotApprovedCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.PreVerificationNotApproved = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.PreVerificationNotApproved.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getPreVerificationNotApproved();
        }
        else {
            $localStorage.Stats = null;
            $scope.PreVerificationNotApproved = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/PreVerificationNotApproved/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/PreVerificationNotApproved/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getPreVerificationNotApproved = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.PreVerificationNotApproved.ProgrammeInstancePartTermId == "" ||
            $scope.PreVerificationNotApproved.ProgrammeInstancePartTermId == null ||
            $scope.PreVerificationNotApproved.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/PreVerificationNotApproved/PendingStatusByFacultyGet',
            data: { ProgrammeInstancePartTermId: $scope.PreVerificationNotApproved.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "PreVerificationNotApprovedFacultyReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'PreVerification NotApproved By Faculty Report | Date and Time: ' + DateAndTime,
            },
            columns: [
