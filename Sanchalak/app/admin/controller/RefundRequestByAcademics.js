app.controller('RefundRequestByAcademicsCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.RefundRequestByAcademics = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.RefundRequestByAcademics.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getRefundRequestByAcademics();
        }
        else {
            $localStorage.Stats = null;
            $scope.RefundRequestByAcademics = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/RefundRequestByAcademics/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/RefundRequestByAcademics/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getRefundRequestByAcademics = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.RefundRequestByAcademics.ProgrammeInstancePartTermId == "" ||
            $scope.RefundRequestByAcademics.ProgrammeInstancePartTermId == null ||
            $scope.RefundRequestByAcademics.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/RefundRequestByAcademics/RefundRequestByAcademicsGet',
            data: { ProgrammeInstancePartTermId: $scope.RefundRequestByAcademics.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.RefundRequestByAcademicsTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "RefundRequestByAcademicsReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'Refund Request By Academics Report | Date and Time: ' + DateAndTime,
            },
            columns: [
