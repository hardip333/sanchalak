app.controller('NotApprovedByFacultyCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.NotApprovedByFaculty = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.NotApprovedByFaculty.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getNotApprovedByFaculty();
        }
        else {
            $localStorage.Stats = null;
            $scope.NotApprovedByFaculty = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/NotApprovedByFaculty/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/NotApprovedByFaculty/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getNotApprovedByFaculty = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.NotApprovedByFaculty.ProgrammeInstancePartTermId == "" ||
            $scope.NotApprovedByFaculty.ProgrammeInstancePartTermId == null ||
            $scope.NotApprovedByFaculty.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/NotApprovedByFaculty/NotApprovedByFacultyGet',
            data: { ProgrammeInstancePartTermId: $scope.NotApprovedByFaculty.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.NotApprovedByFacultyTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "NotApprovedByFacultyReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'Not Approved By Faculty Report | Date and Time: ' + DateAndTime,
            },
            columns: [
