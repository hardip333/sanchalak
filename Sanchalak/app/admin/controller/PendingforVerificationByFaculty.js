app.controller('PendingforVerificationByFacultyCtrl', function ($scope, $http, $filter, $localStorage, $rootScope, $state, $cookies, $mdDialog, NgTableParams) {
    ...
    $scope.PendingforVerificationByFaculty = {}

    $scope.TakeLocalStoradeValue = function () {

        if ($localStorage.Stats.FlagFromAppStats == true) {
            $localStorage.Stats.FlagFromAppStats = false;
            $scope.PendingforVerificationByFaculty.ProgrammeInstancePartTermId = $localStorage.Stats.ProgramInstancePartTermId;
            $scope.getPendingforVerificationByFaculty();
        }
        else {
            $localStorage.Stats = null;
            $scope.PendingforVerificationByFaculty = null;
        }
    };
    ...
    $scope.getIncProgInsPartTermListByInstituteId = function () {
        ...
        $http({
            method: 'Post',
            url: 'api/PendingforVerificationByFaculty/IncProgInsPartTermListGetByInstituteId',
    ...
    $scope.ApplicationListGet = function () {
        ...
        $http({
            method: 'GET',
            url: 'api/PendingforVerificationByFaculty/ApplicationListGet',
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
    ...
    $scope.getPendingforVerificationByFaculty = function () {

        // alert($scope.StudentAdmissionFeesPaidReport.ProgrammeInstancePartTermId);
        //var InsPartTerm = $('#InsPartTerm').val();
        if ($scope.PendingforVerificationByFaculty.ProgrammeInstancePartTermId == "" ||
            $scope.PendingforVerificationByFaculty.ProgrammeInstancePartTermId == null ||
            $scope.PendingforVerificationByFaculty.ProgrammeInstancePartTermId == undefined) {
        ...
        $http({
            method: 'POST',
            url: 'api/PendingforVerificationByFaculty/PendingforVerificationByFacultyGet',
            data: { ProgrammeInstancePartTermId: $scope.PendingforVerificationByFaculty.ProgrammeInstancePartTermId },
            headers: { "Content-Type": 'application/json' }
        })
            .success(function (response) {
                ...
                if (response.response_code != "200") {
                    ...
                    if (response.obj == "The source contains no DataRows.") {
                        ...
                        $scope.PendingforVerificationByFacultyTableParams = new NgTableParams({
                        }, {
                            dataset: null
                        });
    ...
    $scope.exportData = function () {
        ...
        var ExcelFileName = "PendingforVerificationByFacultyReport_" + DateWithoutDashed + time;

        var mystyleSecond = {
            headers: true,
            style: 'font-size:25px;font-weight:bold;',
            caption: {
                title: 'Pending Verification By Faculty Report | Date and Time: ' + DateAndTime,
            },
            columns: [
