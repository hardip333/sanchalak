app.controller('InstGroupChangeRequestCtrl', function ($scope, $http, $rootScope, $filter, $localStorage, $state, $cookies, Upload, $mdDialog, NgTableParams) {
    ...
    $scope.ValidateIFSCCode = function () {
        ...
        if ($scope.InstGroupChange.BankId == 1) {
            $scope.objIFSC = {};
            $scope.objIFSC.IFSCCode = $scope.InstGroupChange.IFSCCODE.substring(0, 5);
    ...
    $scope.InstituteGroupChangeAdd = function () {
		...
		if (($scope.details[0].IsAdmissionFeePaid == true) && ($scope.InstGroupChange.IsFeeCategorySame == "false")) {
			$scope.objIFSC = {};
			$scope.objIFSC.IFSCCode = $scope.InstGroupChange.IFSCCODE.substring(0, 5);
			}
		else {
			$scope.objIFSC = {};
			$scope.objIFSC.IFSCCode = null;
		}
       
        if (($scope.details[0].IsAdmissionFeePaid == true) && ($localStorage.AddPassBookImage != null || $localStorage.AddPassBookImage != undefined || $localStorage.AddPassBookImage != "")) {
            $cookies.put("passbookImage", $localStorage.AddPassBookImage);
        }

        if (($scope.details[0].IsAdmissionFeePaid == true) && ($localStorage.AddRTGSForm != null || $localStorage.AddRTGSForm != undefined || $localStorage.AddRTGSForm != "")) {
            $cookies.put("InstGroupChangeRTGSForm", $localStorage.AddRTGSForm);
        }

        if ($scope.InstGroupChange.IsFeeCategorySame == "false" && $scope.details[0].IsAdmissionFeePaid == true && $scope.objIFSC.IFSCCode != "BARB0") {
            ...
            var RTGS = $("#SuccessMsgRTGS").text();

            if (RTGS == null || RTGS == undefined || RTGS == "") {
                alert("File Not Uploaded Or File is Not in Proper Format");
                return false;
            }
            else {
                $scope.InstGroupChange.RTGSForm = $scope.AddRTGSForm;
            }
        }
        ...
        if ($scope.InstGroupChange.IsFeeCategorySame == "false" && $scope.details[0].IsAdmissionFeePaid == true && $scope.objIFSC.IFSCCode == "BARB0") {
            ...
            $scope.InstGroupChange.RTGSForm = null;
        }        

        if ($scope.InstGroupChange.IsFeeCategorySame == "false" && $scope.details[0].IsAdmissionFeePaid == true && ($scope.objIFSC.IFSCCode == "BARB0" && $scope.InstGroupChange.BankId != 1)) {
            $scope.ValidateIFSCCode();
            alert("Please Enter Valid IFSC Code or Bank Name.!!");       

        }
        
        else if (($scope.details[0].IsAdmissionFeePaid == true) && ($scope.InstGroupChange.BankId == 1 && $scope.objIFSC.IFSCCode != "BARB0")) {
            $scope.ValidateIFSCCode();            
        }
        ...
        $scope.InstGroupChange1.PassbookPhoto = $scope.InstGroupChange.PassbookPhoto;
            $scope.InstGroupChange1.RTGSForm = $scope.InstGroupChange.RTGSForm;
        $scope.InstGroupChange1.IsPRNGenerated = $scope.details[0].IsPRNGenerated;
            $scope.InstGroupChange1.IFSCShortCode = $scope.objIFSC.IFSCCode;
    ...
    $scope.UploadStudentPassBookImage = function ($files) {
        ...
        $scope.AddPassBookImage = ApplicantUserName + "_" + "PassBook_InstGroupChange" + "_" + date + "_" + month + "_" + year + fileExtension;

        $localStorage.AddPassBookImage = $scope.AddPassBookImage;
    ...
    $scope.UploadStudentRTGSForm = function ($files) {
        ...
        var fileExtension = '.' + $scope.SelectedFiles1[0].name.split('.').pop();
        //var name = $scope.SelectedFiles[0].name.split('.').slice(0, -1);
        $scope.AddRTGSForm = ApplicantUserName + "_" + "RTGSForm_InstGroupChange" + "_" + date + "_" + month + "_" + year + fileExtension;

        $localStorage.AddRTGSForm = $scope.AddPassBookImage;

        $cookies.put("InstGroupChangeRTGSForm", $scope.AddRTGSForm);
        ...
        if ($scope.SelectedFiles1 && $scope.SelectedFiles1.length) {
            ...
            if (!allowedExtensions.exec(fileCheck)) {
                ...
                document.getElementById("ErrorMsgRTGS").innerHTML = "It only accepts Pdf file";
                document.getElementById("SuccessMsgRTGS").innerHTML = "";
                fileCheck.value = '';
                return false;
            }
            ...
            if (fi.files.length > 0) {
                ...
                $scope.byId(5).then(function (response) {
                    ...
                    for (i = 0; i < cnt; i++) {
                        ...
                        const file = Math.round((fsize / 1024));
                        if (file >= $scope.maxVal) {
                            document.getElementById("ErrorMsgRTGS").innerHTML = "Please select a file below 512KB";
                            document.getElementById("SuccessMsgRTGS").innerHTML = "";
                            return false;
                        }
                        else {
                            Upload.upload({
                                url: 'api/InstituteGroupChange/UploadRTGSForm',
                                data: {
                                ...
                                if (response.data.response_code == "200") {
                                    ...
                                    document.getElementById("ErrorMsgRTGS").innerHTML = "";
                                    document.getElementById("SuccessMsgRTGS").innerHTML = "File Uploaded Successfully - Click Submit To Continue";
                                }
                                else {

                                    $scope.AddRTGSForm = undefined;
                                    $("#file2").val("");
                                    alert("File Upload Failed!");
                                    return false;
                                }
