...
{
    public class NotApprovedByFacultyController : ApiController
    {
        ...
        #region NoApprovedByFacultyReportGet
        [HttpPost]
        public HttpResponseMessage NotApprovedByFacultyGet(NotApprovedByFaculty AFR)
        {
            try
            {
                ...
                SqlCommand Cmd = new SqlCommand("NotApprovedByFacultyReport", Con);
                ...
                Int32 count = 0;
                List<NotApprovedByFaculty> ObjLstAFR = new List<NotApprovedByFaculty>();

                if (Dt.Rows.Count > 0)
                {
                    for (int i = 0; i < Dt.Rows.Count; i++)
                    {
                        NotApprovedByFaculty ObjAFR = new NotApprovedByFaculty();
                        ...
                        //ObjAFR.EligibilityByFaculty = (Convert.ToString(Dt.Rows[i]["EligibilityByFaculty"])).IsEmpty() ? "Not Defined" : Convert.ToString(Dt.Rows[i]["EligibilityByFaculty"]);
