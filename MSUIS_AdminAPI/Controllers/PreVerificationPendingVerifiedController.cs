...
{
    public class PreVerificationPendingVerifiedController : ApiController
    {
        ...
        #region PreVerificationPendingVerifiedGet
        [HttpPost]
        public HttpResponseMessage PreVerificationPendingVerifiedGet(PreVerificationPendingVerified PSF)
        {
            try
            {
                ...
                SqlCommand Cmd = new SqlCommand("PreVerificationPendingVerified", Con);
                Cmd.CommandType = CommandType.StoredProcedure;


                Cmd.Parameters.AddWithValue("@ProgrammeInstancePartTermId", PSF.ProgrammeInstancePartTermId);
                ...
                Int32 count = 0;
                List<PreVerificationPendingVerified> ObjLstVF = new List<PreVerificationPendingVerified>();

                if (Dt.Rows.Count > 0)
                {
                    for (int i = 0; i < Dt.Rows.Count; i++)
                    {
                        PreVerificationPendingVerified ObjVF = new PreVerificationPendingVerified();
                        ...
                        //ObjVF.EligibilityStatus = (Convert.ToString(Dt.Rows[i]["EligibilityStatus"])).IsEmpty() ? "Not Defined" : Convert.ToString(Dt.Rows[i]["EligibilityStatus"]);
